...
export const sendRequest = async (data, io, socket) => {
  try {
    ...
    Object.values(io.of("/api/friends").sockets).forEach((socket) =>
      console.log("socket" + socket.username + "socket id" + socket.id)
    );
...
export const acceptRequest = async (data, io, socket) => {
  try {
    ...
    if (sender) {
      ...
      if (
        !sender.friends.includes(socket.username) &&
        !receiver.friends.includes(data.username)
      ) {
        sender.friends.push(socket.username);

        receiver.friends.push(data.username);

        sender.outgoingFriendRequests.splice(
          sender.outgoingFriendRequests.indexOf(socket.username),
          1
        );

        receiver.incomingFriendRequests.splice(
          receiver.incomingFriendRequests.indexOf(data.username),
          1
        );
        ...
        Object.values(io.of("/api/friends").sockets).forEach((sock) => {
          ...
          if (sock.username === data.username) {
            ...
            socket
              .to(sock.id)
              .emit("friend request accepted", { username: socket.username });
          }
        });
      }
    }
...
export const deleteFriend = async (data, io, socket) => {
  try {
    ...
    if (!friend)
      return socket.emit("notify", {
        message: "User to unfriend doesn't exist",
      });
    ...
    if (
      ...
      return socket.emit("notify", { message: "User is not your friend" });
...
export const declineRequest = async (data, io, socket) => {
  try {
    if (socket.username === data.username) return;
    ...
    if (sender) {
      ...
      if (
        sender.outgoingFriendRequests.includes(socket.username) &&
        receiver.incomingFriendRequests.includes(data.username)
      ) {
        sender.outgoingFriendRequests.splice(
          sender.outgoingFriendRequests.indexOf(socket.username),
          1
        ...
        socket.emit("notify", { message: "Friend request declined." });
        ...
        Object.values(io.of("/api/friends").sockets).forEach((sock) => {
          ...
          if (sock.username === data.username) {
            ...
            socket
              .to(sock.id)
              .emit("notify", {
                message: `${socket.username} declined your friend request.`,
                username: socket.username,
              });
          }
        });
